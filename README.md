...
- **Config (slave):** `USE_DOCKER_QUOTA`, `DOCKER_DATA_ROOT` (optional), `DOCKER_QUOTA_RESERVED_BYTES` (optional), `CELERY_BROKER_URL`, `DOCKER_QUOTA_ENFORCE_INTERVAL_SECONDS` (default 300), `DOCKER_QUOTA_SYNC_INTERVAL_SECONDS` (default 600), `DOCKER_VOLUME_ACTUAL_DISK_SYNC_INTERVAL_SECONDS` (default 21600 = 6h, for actual disk scan), `DOCKER_VOLUME_ACTUAL_DISK_TIMEOUT_PER_VOLUME_SECONDS` (optional, default 3600), `DOCKER_QUOTA_ENFORCEMENT_ORDER` (default `newest_first`), `SLAVE_HOST_ID`, `MASTER_EVENT_CALLBACK_URL`, `MASTER_EVENT_CALLBACK_SECRET`. See `config.slave_docker_example.json`.
