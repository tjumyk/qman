...
When **USE_DOCKER_QUOTA** is `true`, the slave exposes a virtual "Docker" device (same device-list shape as block/ZFS). Attribution of containers to users is by label **`qman.user=username`** at create time and/or by **auditd + Docker events** (periodic sync task). Set quota via the same API with `device=docker`. Enforcement runs in a Celery worker: over-quota users have **at most one running container stopped per beat** (containers are not removed). Order is configurable (**`DOCKER_QUOTA_ENFORCEMENT_ORDER`**: `newest_first` (default), `oldest_first`, or `largest_first`). Usage semantics are consistent with pyquota/ZFS: `used` = attributed + unattributed, `used + free = total`. Unattributed usage is shown separately in the UI usage breakdown as "Other". Events (quota exceeded, container stopped) can be sent to the master for email notifications.

- **Config (slave):** `USE_DOCKER_QUOTA`, `DOCKER_DATA_ROOT` (optional), `DOCKER_QUOTA_RESERVED_BYTES` (optional), `CELERY_BROKER_URL`, `CELERY_TIMEZONE` (optional, default `UTC`), `DOCKER_QUOTA_ENFORCE_INTERVAL_SECONDS` (default 300), `DOCKER_QUOTA_SYNC_INTERVAL_SECONDS` (default 600), `DOCKER_VOLUME_ACTUAL_DISK_SYNC_INTERVAL_SECONDS` (default 600 = 10 minutes, for actual disk scan), `DOCKER_VOLUME_ACTUAL_DISK_TIMEOUT_PER_VOLUME_SECONDS` (optional, default 3600), `DOCKER_QUOTA_ENFORCEMENT_ORDER` (default `newest_first`), `SLAVE_HOST_ID`, `MASTER_EVENT_CALLBACK_URL`, `MASTER_EVENT_CALLBACK_SECRET`. See `config.slave_docker_example.json`.
