import {
  createContext,
  useCallback,
  useContext,
  useMemo,
  useState,
  type ReactNode,
} from 'react'

const STORAGE_KEY = 'qman-locale'

export type Locale = 'en' | 'zh-Hans'

function getDefaultLocale(): Locale {
  if (typeof window === 'undefined') return 'en'
  const stored = localStorage.getItem(STORAGE_KEY) as Locale | null
  if (stored === 'zh-Hans' || stored === 'en') return stored
  const browser = navigator.language
  return browser.startsWith('zh') ? 'zh-Hans' : 'en'
}

const translations: Record<Locale, Record<string, string>> = {
  en: {
    appTitle: 'Qman',
    appDescription: 'Disk Quota Management System',
    loading: 'Loading...',
    notSignedIn: 'Not signed in',
    pleaseLogin: 'Please log in to use Quota Manager.',
    myUsage: 'My usage',
    dashboard: 'Dashboard',
    hostList: 'Host list',
    myProfile: 'My profile',
    logOut: 'Log out',
    footer: '©Kelvin, 2026.',
    themeLight: 'Light',
    themeDark: 'Dark',
    langEn: 'En',
    langZh: '中文',
    admin: 'admin',
    // Status
    statusOk: 'OK',
    statusOverSoftLimit: 'Over soft limit',
    statusOverHardLimit: 'Over hard limit',
    online: 'Online',
    offline: 'Offline',
    // Errors & loading
    error: 'Error',
    failedToLoadQuotas: 'Failed to load quotas',
    missingHost: 'Missing host',
    missingHostOrDevice: 'Missing host or device',
    deviceNotFound: 'Device not found',
    hostNotFoundOrError: 'Host not found or error',
    loadingYourQuotas: 'Loading your quotas...',
    loadingTakingLonger: 'Likely a cache miss. Collecting data from all hosts. Please wait, it usually takes about 30 seconds.',
    // Pages & sections
    hosts: 'Hosts',
    devices: 'Devices',
    devicesWithQuota: 'Devices (with quota)',
    userQuotas: 'User quotas',
    totalHosts: 'Total hosts',
    totalDevices: 'Total devices',
    usersOverSoft: 'Users over soft limit',
    usersOverHard: 'Users over hard limit',
    needsAttention: 'Needs attention',
    noUsersOverLimit: 'No users over soft or hard limit.',
    noQuotas: 'No quotas',
    noQuotasAssigned: 'You have no quota assignments on any host.',
    noQuotasAssignedInHost: 'This host has no quota assignments.',
    yourQuotaUsage: 'Your quota usage',
    noUsersMatch: 'No users match.',
    needAttentionCount: 'need attention',
    // Table & form
    searchByNameOrUid: 'Search by name or uid...',
    filterByStatus: 'Filter by status',
    edit: 'Edit',
    cancel: 'Cancel',
    save: 'Save',
    actions: 'Actions',
    editQuota: 'Edit quota',
    blockSoftLimit1k: 'Block soft limit (1K blocks)',
    blockHardLimit1k: 'Block hard limit (1K blocks)',
    inodeSoftLimit: 'Inode soft limit',
    inodeHardLimit: 'Inode hard limit',
    quotaUpdated: 'Quota updated',
    failedToUpdateQuota: 'Failed to update quota',
    addQuota: 'Add quota',
    addQuotaForUser: 'Add quota for user',
    usernameLabel: 'Username',
    continue: 'Continue',
    userNotFound: 'User not found',
    blockUsage: 'Block usage',
    inodeUsage: 'Inode usage',
    // Counts (use with number: e.g. "5 device(s)")
    deviceCount: 'device(s)',
    userCount: 'user(s)',
    // Table headers
    uid: 'uid',
    name: 'name',
    blockUsed: 'block used',
    blockSoft: 'block soft',
    blockHard: 'block hard',
    inodeUsed: 'inode used',
    inodeSoft: 'inode soft',
    inodeHard: 'inode hard',
    status: 'status',
    // Host detail
    hostDevicesTitle: 'devices',
    // Device info
    fstypeLabel: 'Filesystem type',
    mountPointsLabel: 'Mount points',
    diskUsageLabel: 'Disk usage',
    usedLabel: 'Used',
    reservedSoftLabel: 'Quota reserved (soft)',
    reservedHardLabel: 'Quota reserved (hard)',
    reservedLabel: 'Quota reserved',
    trackedUsageLabel: 'User usage',
    otherUsageLabel: 'Other usage (root + system)',
    rootReservedLabel: 'Filesystem reserved',
    freeSpaceLabel: 'Free',
    captionEqualsTotal: 'Total',
    captionEqualsDemand: 'Demand',
    captionPercentUsed: 'used',
    noFreeSpaceLabel: 'No free space',
    overSoldLabel: 'Over-sold',
    overUseLabel: 'Over-use',
    diskLimitLabel: 'Disk size',
    softQuotaUsageLabel: 'Quota usage (soft)',
    hardQuotaUsageLabel: 'Quota usage (hard)',
    quotaUsageLabel: 'Quota usage',
    // Mappings
    myMappings: 'My mappings',
    manageMyMappings: 'Manage my host users',
    noHostUserLinked: 'No host user linked',
    noHostUserLinkedMessage: 'No host user is linked to your account. Link a host user to see your quota usage.',
    hostUser: 'Host user',
    myMappingsDescription: 'Link your account to host users to see your quota on "My usage".',
    addMapping: 'Add mapping',
    addUser: 'Add user',
    removeMapping: 'Remove',
    noMappings: 'No mappings',
    noMappingsMessage: 'You have no host user mappings. Add one to see your quota on My usage.',
    selectHost: 'Select host',
    selectHostUser: 'Select host user',
    selectHostFirst: 'Select a host first',
    suggested: 'suggested',
    failedToLoadMappings: 'Failed to load mappings',
    failedToAddMapping: 'Failed to add mapping',
    failedToRemoveMapping: 'Failed to remove mapping',
    userMappings: 'User mappings',
    userMappingsDescription: 'OAuth–host user mappings: which OAuth users can see which host users\' quota.',
    mappedOAuthUsers: 'Mapped OAuth users',
    oauthUser: 'OAuth user',
    loadOAuthUsers: 'Load OAuth users',
    loadOAuthUsersFirst: 'Load OAuth users',
    noAdminMappingsMessage: 'No host users or mappings yet.',
    host: 'Host',
    deviceTypeDocker: 'Docker',
    containerRemovedDueToQuota: 'Container removed due to quota',
    dockerUsageContainerWorkLayer: 'Container work layer',
    dockerUsageImageLayers: 'Image layers',
    dockerUsageVolumes: 'Volumes',
    dockerQuotaRemainingLabel: 'Remaining quota',
    // Docker detail tabs
    usersTab: 'Users',
    containersTab: 'Containers',
    imagesTab: 'Images',
    volumesTab: 'Volumes',
    // Docker summary cards
    totalBytes: 'Total',
    attributedBytes: 'Attributed',
    unattributedBytes: 'Unattributed',
    ofTotal: 'of total',
    // Docker containers
    containerId: 'Container ID',
    containerName: 'Name',
    containerImage: 'Image',
    containerStatus: 'Status',
    owner: 'Owner',
    size: 'Size',
    unattributed: 'Unattributed',
    searchContainers: 'Search containers...',
    noContainersMatch: 'No containers match.',
    failedToLoadDockerContainers: 'Failed to load Docker containers',
    // Docker images
    imageId: 'Image ID',
    imageTags: 'Tags',
    created: 'Created',
    imagesSection: 'Images',
    layersSection: 'Layers',
    searchImages: 'Search images...',
    noImagesMatch: 'No images match.',
    noTags: 'No tags',
    more: 'more',
    failedToLoadDockerImages: 'Failed to load Docker images',
    // Docker layers
    layerId: 'Layer ID',
    firstPuller: 'First Puller',
    creationMethod: 'Method',
    firstSeen: 'First Seen',
    searchLayers: 'Search layers...',
    noLayersMatch: 'No layers match.',
    hideZeroSizeLayers: 'Hide layers with 0 size',
    // Docker volumes
    volumeName: 'Volume Name',
    refCount: 'Ref Count',
    scan: 'Scan',
    lastMountedAt: 'Last mounted',
    neverScanned: 'Never scanned',
    attributionSource: 'Source',
    searchVolumes: 'Search volumes...',
    noVolumesMatch: 'No volumes match.',
    failedToLoadDockerVolumes: 'Failed to load Docker volumes',
    // Edit quota
    currentUsage: 'Current Usage',
    noLimit: 'No Limit',
    matchCurrentUsage: 'Match Current Usage',
    usageVsNewLimit: 'Usage vs. New Limit',
    overLimitWarning: 'Over limit!',
    softExceedsHardError: 'Soft limit cannot exceed hard limit',
    // Quota presets
    presetNoLimit: 'No Limit',
    presetMatchUsage: '= Usage',
    presetMinus2G: '−2G',
    presetPlus2G: '+2G',
    presetMinus10G: '−10G',
    presetPlus10G: '+10G',
    presetDouble: '2×',
    // Batch quota
    batchSetQuota: 'Batch Set Quota',
    quotaLimit: 'Quota Limit',
    preserveOptions: 'Preserve Options',
    preserveIfNonzero: 'Preserve existing quota if non-zero',
    preserveIfUsageExceeds: 'Preserve if current usage exceeds new default',
    preview: 'Preview',
    totalHostUsers: 'Total host users',
    usersToUpdate: 'Users to update',
    usersToSkip: 'Users to skip',
    totalQuotaAllocation: 'Total quota allocation',
    deviceTotal: 'Device total',
    utilizationPercent: 'Utilization',
    apply: 'Apply',
    batchQuotaSuccess: 'Batch quota applied: {updated}/{total} users updated',
    batchQuotaFailed: 'Failed to apply batch quota',
    defaultQuota: 'Default quota',
    editDefaultQuota: 'Edit default quota',
    defaultQuotaNotSet: 'No default quota set for this device.',
    defaultQuotaSaved: 'Default quota saved. A background worker will apply it to users with no limits.',
    defaultQuotaSaveFailed: 'Failed to save default quota',
    defaultQuotaUsageMessage:
      'A background task runs periodically and applies this default to users on this device who have no limits set. Users who already have limits are not changed.',
    aboveDefault: 'Above default',
    fillFromDefault: 'Fill from default',
    blockGrace: 'Block grace',
    inodeGrace: 'Inode grace',
    graceEnds: 'ends',
    graceExpired: 'Expired',
    graceTooltipActive:
      'You are over the soft limit. Reduce your usage before this time, or writes may be blocked when the grace period ends.',
    graceTooltipExpired:
      'The grace period has ended and you are still over the soft limit. Reduce your usage now to avoid write blocks, or contact your administrator.',
    graceDurationDays: '{n} days',
    graceDurationHours: '{n} hours',
    graceDurationMinutes: '{n} minutes',
    graceDurationSeconds: '{n} seconds',
    // Auto-assign mappings
    autoAssign: 'Auto-assign',
    autoAssignMappings: 'Auto-assign Mappings',
    autoAssignDescription: 'The following host users have OAuth users with exactly matching names that are not yet mapped. Select the mappings you want to create.',
    noCandidateMappings: 'No candidate mappings found. All host users with matching OAuth user names are already mapped.',
    confirmMappings: 'Confirm Mappings',
    candidateMappingsCount: '{count} candidate mapping(s) found',
    selectAll: 'Select all',
    deselectAll: 'Deselect all',
    suggestedMappings: 'Suggested Mappings',
    suggestedMappingsDescription: 'The same mapping ({oauthUser} → {hostUser}) can be applied to other hosts. Select the ones you want to add.',
    // Notification center
    notificationCenter: 'Notification center',
    notificationCenterDescription: 'Historical log of quota-related notification emails (success, failed, or skipped).',
    notifications: 'Notifications',
    events: 'Events',
    eventType: 'Event type',
    sendStatus: 'Send status',
    email: 'Email',
    subject: 'Subject',
    details: 'Details',
    sendStatusSuccess: 'Success',
    sendStatusFailed: 'Failed',
    sendStatusSkipped: 'Skipped',
    noNotifications: 'No notifications have been recorded yet.',
    failedToLoadNotifications: 'Failed to load notifications',
    bodyPreview: 'Body preview',
    payloadJson: 'Payload JSON',
    time: 'Time',
    // Docker usage review (admin)
    dockerUsageReviewNav: 'Docker review',
    dockerUsageReviewTitle: 'Docker usage review',
    dockerUsageReviewDescription:
      'Inspect unresolved Docker/audit events on a host and assign manual attribution (overrides auto-attribution for quota).',
    dockerUsageReviewPickHost: 'Select host',
    dockerUsageReviewPickHostHint: 'Choose a host to load the review queue.',
    dockerUsageReviewEntityType: 'Entity type',
    dockerUsageReviewContainers: 'Containers',
    dockerUsageReviewImages: 'Images',
    dockerUsageReviewVolumes: 'Volumes',
    dockerUsageReviewQueueFailed: 'Failed to load review queue',
    dockerUsageReviewEmpty: 'No entities with unresolved events for this filter.',
    dockerUsageReviewColId: 'ID',
    dockerUsageReviewColUnresolved: 'Unresolved',
    dockerUsageReviewColAttribution: 'Attribution',
    dockerUsageReviewDetails: 'Details',
    dockerUsageReviewDrawerTitle: 'Entity detail',
    dockerUsageReviewCurrentAttribution: 'Current attribution',
    dockerUsageReviewIncludeUsed: 'Include events already used for auto-attribution',
    dockerUsageReviewEventsFailed: 'Failed to load events',
    dockerUsageReviewColSource: 'Source',
    dockerUsageReviewColWhen: 'When',
    dockerUsageReviewColPayload: 'Payload',
    dockerUsageReviewAssignee: 'Assign to mapped user',
    dockerUsageReviewAssigneePlaceholder: 'Choose OAuth → host user mapping',
    dockerUsageReviewNoMappings: 'No mappings for this host. Add mappings under User mappings.',
    dockerUsageReviewCascade: 'Cascade to related image/layers/volumes (container/image only)',
    dockerUsageReviewAssign: 'Assign',
    dockerUsageReviewClear: 'Clear manual assignment',
    dockerUsageReviewAssigned: 'Manual attribution saved',
    dockerUsageReviewAssignFailed: 'Failed to assign attribution',
    dockerUsageReviewCleared: 'Manual attribution cleared',
    dockerUsageReviewClearFailed: 'Failed to clear attribution',
    dockerEventColReview: 'Review',
    dockerEventColAuto: 'Auto attribution',
    dockerEventReview_pending: 'Pending',
    dockerEventReview_reviewed: 'Reviewed',
    dockerEventAuto_unused: 'Unused',
    dockerEventAuto_consumed: 'Consumed',
    dockerEventPayloadModalTitle: 'Event payload (JSON)',
    dockerEntityDetailTitle: 'Docker entity detail',
    dockerEntityDetailTabInspect: 'Inspect',
    dockerEntityDetailTabAttribution: 'Attribution',
    dockerEntityDetailTabEvents: 'Events',
    dockerEntityDetailAutoAttribution: 'Auto attribution (database)',
    dockerEntityDetailManualOverride: 'Manual override',
    dockerEntityDetailNoAuto: 'No auto-attribution row for this entity.',
    dockerEntityDetailNoOverride: 'No manual override; effective quota uses auto only.',
    dockerEntityDetailIncludeUsed: 'Include events already consumed for auto-attribution',
    dockerEntityDetailIncludeResolved: 'Include events already cleared from review',
    dockerEntityDetailInspectFailed: 'Failed to load docker inspect',
    dockerEntityDetailAttributionFailed: 'Failed to load attribution detail',
    dockerEntityDetailEventsFailed: 'Failed to load usage events',
    dockerEntityDetailDetails: 'Details',
  },
  'zh-Hans': {
    appTitle: 'Qman',
    appDescription: '磁盘配额管理系统',
    loading: '加载中…',
    notSignedIn: '未登录',
    pleaseLogin: '请登录以使用配额管理。',
    myUsage: '我的用量',
    dashboard: '仪表盘',
    hostList: '主机列表',
    myProfile: '个人资料',
    logOut: '退出登录',
    footer: '©Kelvin, 2026.',
    themeLight: '浅色',
    themeDark: '深色',
    langEn: 'En',
    langZh: '中文',
    admin: '管理员',
    statusOk: '正常',
    statusOverSoftLimit: '超过软限',
    statusOverHardLimit: '超过硬限',
    online: '在线',
    offline: '离线',
    error: '错误',
    failedToLoadQuotas: '加载配额失败',
    missingHost: '缺少主机',
    missingHostOrDevice: '缺少主机或设备',
    deviceNotFound: '未找到设备',
    hostNotFoundOrError: '未找到主机或出错',
    loadingYourQuotas: '正在加载您的配额…',
    loadingTakingLonger: '缓存可能未命中，正在从所有主机收集信息。请稍等片刻，这一般在 30 秒内完成。',
    hosts: '主机',
    devices: '设备',
    devicesWithQuota: '设备（含配额）',
    userQuotas: '用户配额',
    totalHosts: '主机总数',
    totalDevices: '设备总数',
    usersOverSoft: '超过软限制的用户',
    usersOverHard: '超过硬限制的用户',
    needsAttention: '需关注',
    noUsersOverLimit: '没有超过软/硬限制的用户。',
    noQuotas: '无配额',
    noQuotasAssigned: '您在任何主机上都没有配额分配。',
    noQuotasAssignedInHost: '该主机上没有配额分配。',
    yourQuotaUsage: '您的配额用量',
    noUsersMatch: '没有匹配的用户。',
    needAttentionCount: '需关注',
    searchByNameOrUid: '按姓名或 uid 搜索…',
    filterByStatus: '按状态筛选',
    edit: '编辑',
    cancel: '取消',
    save: '保存',
    actions: '操作',
    editQuota: '编辑配额',
    blockSoftLimit1k: '块软限制（1K 块）',
    blockHardLimit1k: '块硬限制（1K 块）',
    inodeSoftLimit: 'Inode 软限制',
    inodeHardLimit: 'Inode 硬限制',
    quotaUpdated: '配额已更新',
    failedToUpdateQuota: '更新配额失败',
    addQuota: '添加配额',
    addQuotaForUser: '为用户添加配额',
    usernameLabel: '用户名',
    continue: '继续',
    userNotFound: '用户未找到',
    blockUsage: '块用量',
    inodeUsage: 'Inode 用量',
    deviceCount: '台设备',
    userCount: '个用户',
    uid: 'uid',
    name: '名称',
    blockUsed: '块已用',
    blockSoft: '块软限',
    blockHard: '块硬限',
    inodeUsed: 'inode 已用',
    inodeSoft: 'inode 软限',
    inodeHard: 'inode 硬限',
    status: '状态',
    hostDevicesTitle: '设备',
    fstypeLabel: '文件系统类型',
    mountPointsLabel: '挂载点',
    diskUsageLabel: '磁盘用量',
    usedLabel: '已用',
    reservedSoftLabel: '配额预留（软限）',
    reservedHardLabel: '配额预留（硬限）',
    reservedLabel: '配额预留',
    trackedUsageLabel: '用户用量',
    otherUsageLabel: '其他用量（root + 系统）',
    rootReservedLabel: '文件系统预留',
    freeSpaceLabel: '可用',
    captionEqualsTotal: '总量',
    captionEqualsDemand: '需求总量',
    captionPercentUsed: '已用',
    noFreeSpaceLabel: '无可用空间',
    overSoldLabel: '超额分配',
    overUseLabel: '超量使用',
    diskLimitLabel: '磁盘容量',
    softQuotaUsageLabel: '配额用量（软限）',
    hardQuotaUsageLabel: '配额用量（硬限）',
    quotaUsageLabel: '配额用量',
    myMappings: '我的映射',
    manageMyMappings: '管理我的主机用户',
    noHostUserLinked: '未关联主机用户',
    noHostUserLinkedMessage: '您的账户尚未关联任何主机用户。关联后可在此查看配额用量。',
    hostUser: '主机用户',
    myMappingsDescription: '将账户与主机用户关联后，可在「我的用量」中查看配额。',
    addMapping: '添加映射',
    addUser: '添加用户',
    removeMapping: '移除',
    noMappings: '无映射',
    noMappingsMessage: '您还没有任何主机用户映射。添加后可查看配额。',
    selectHost: '选择主机',
    selectHostUser: '选择主机用户',
    selectHostFirst: '请先选择主机',
    suggested: '推荐',
    failedToLoadMappings: '加载映射失败',
    failedToAddMapping: '添加映射失败',
    failedToRemoveMapping: '移除映射失败',
    userMappings: '用户映射',
    userMappingsDescription: 'OAuth 用户与主机用户的映射关系。',
    mappedOAuthUsers: '已映射的 OAuth 用户',
    oauthUser: 'OAuth 用户',
    loadOAuthUsers: '加载 OAuth 用户',
    loadOAuthUsersFirst: '加载 OAuth 用户',
    noAdminMappingsMessage: '暂无主机用户或映射。',
    host: '主机',
    deviceTypeDocker: 'Docker',
    containerRemovedDueToQuota: '因配额移除容器',
    dockerUsageContainerWorkLayer: '容器可写层',
    dockerUsageImageLayers: '镜像层',
    dockerUsageVolumes: '数据卷',
    dockerQuotaRemainingLabel: '剩余配额',
    unattributedUsageLabel: '未归属（无 qman.user）',
    // Docker detail tabs
    usersTab: '用户',
    containersTab: '容器',
    imagesTab: '镜像',
    volumesTab: '数据卷',
    // Docker summary cards
    totalBytes: '总量',
    attributedBytes: '已归属',
    unattributedBytes: '未归属',
    ofTotal: '占总量',
    // Docker containers
    containerId: '容器 ID',
    containerName: '名称',
    containerImage: '镜像',
    containerStatus: '状态',
    owner: '所有者',
    size: '大小',
    unattributed: '未归属',
    searchContainers: '搜索容器...',
    noContainersMatch: '没有匹配的容器。',
    failedToLoadDockerContainers: '加载 Docker 容器失败',
    // Docker images
    imageId: '镜像 ID',
    imageTags: '标签',
    created: '创建时间',
    imagesSection: '镜像',
    layersSection: '层',
    searchImages: '搜索镜像...',
    noImagesMatch: '没有匹配的镜像。',
    noTags: '无标签',
    more: '更多',
    failedToLoadDockerImages: '加载 Docker 镜像失败',
    // Docker layers
    layerId: '层 ID',
    firstPuller: '首次拉取者',
    creationMethod: '方式',
    firstSeen: '首次出现',
    searchLayers: '搜索层...',
    noLayersMatch: '没有匹配的层。',
    hideZeroSizeLayers: '隐藏大小为 0 的层',
    // Docker volumes
    volumeName: '数据卷名称',
    refCount: '引用数',
    scan: '扫描',
    lastMountedAt: '最后挂载',
    neverScanned: '从未扫描',
    attributionSource: '来源',
    searchVolumes: '搜索数据卷...',
    noVolumesMatch: '没有匹配的数据卷。',
    failedToLoadDockerVolumes: '加载 Docker 数据卷失败',
    // Edit quota
    currentUsage: '当前用量',
    noLimit: '无限制',
    matchCurrentUsage: '匹配当前用量',
    usageVsNewLimit: '用量 vs. 新限制',
    overLimitWarning: '超出限制！',
    softExceedsHardError: '软限制不能超过硬限制',
    // Quota presets
    presetNoLimit: '无限制',
    presetMatchUsage: '= 用量',
    presetMinus2G: '−2G',
    presetPlus2G: '+2G',
    presetMinus10G: '−10G',
    presetPlus10G: '+10G',
    presetDouble: '2×',
    // Batch quota
    batchSetQuota: '批量设置配额',
    quotaLimit: '配额限制',
    preserveOptions: '保留选项',
    preserveIfNonzero: '如果已有配额非零则保留',
    preserveIfUsageExceeds: '如果当前用量超过新默认值则保留',
    preview: '预览',
    totalHostUsers: '主机用户总数',
    usersToUpdate: '将更新的用户',
    usersToSkip: '将跳过的用户',
    totalQuotaAllocation: '配额分配总量',
    deviceTotal: '设备总容量',
    utilizationPercent: '利用率',
    apply: '应用',
    batchQuotaSuccess: '批量配额已应用：{updated}/{total} 个用户已更新',
    defaultQuota: '默认配额',
    editDefaultQuota: '编辑默认配额',
    defaultQuotaNotSet: '此设备未设置默认配额。',
    defaultQuotaSaved: '默认配额已保存。后台任务会将其应用于无限制的用户。',
    defaultQuotaSaveFailed: '保存默认配额失败',
    defaultQuotaUsageMessage:
      '后台任务会定期运行，并将此默认值应用于本设备上尚未设置限额的用户。已有限额的用户不会被修改。',
    aboveDefault: '高于默认值',
    fillFromDefault: '从默认填充',
    blockGrace: '块宽限',
    inodeGrace: 'Inode 宽限',
    graceEnds: '结束',
    graceExpired: '已过期',
    graceTooltipActive:
      '您已超过软限制。请在此时间前减少用量，否则宽限期结束后写入可能被阻止。',
    graceTooltipExpired:
      '宽限期已结束，您仍超过软限制。请立即减少用量以避免写入被阻止，或联系管理员。',
    graceDurationDays: '{n} 天',
    graceDurationHours: '{n} 小时',
    graceDurationMinutes: '{n} 分钟',
    graceDurationSeconds: '{n} 秒',
    batchQuotaFailed: '批量配额应用失败',
    // Auto-assign mappings
    autoAssign: '自动分配',
    autoAssignMappings: '自动分配映射',
    autoAssignDescription: '以下主机用户有名称完全匹配但尚未映射的 OAuth 用户。选择要创建的映射。',
    noCandidateMappings: '未找到候选映射。所有主机用户已与匹配的 OAuth 用户映射。',
    confirmMappings: '确认映射',
    candidateMappingsCount: '找到 {count} 个候选映射',
    selectAll: '全选',
    deselectAll: '取消全选',
    suggestedMappings: '推荐映射',
    suggestedMappingsDescription: '相同的映射（{oauthUser} → {hostUser}）可以应用到其他主机。选择要添加的映射。',
    // Notification center
    notificationCenter: '通知中心',
    notificationCenterDescription: '配额相关邮件通知的历史记录（成功、失败或跳过）。',
    notifications: '通知',
    events: '事件',
    eventType: '事件类型',
    sendStatus: '发送状态',
    email: '邮箱',
    subject: '主题',
    details: '详情',
    sendStatusSuccess: '成功',
    sendStatusFailed: '失败',
    sendStatusSkipped: '已跳过',
    noNotifications: '当前没有任何通知记录。',
    failedToLoadNotifications: '加载通知记录失败',
    bodyPreview: '邮件内容预览',
    payloadJson: 'Payload JSON',
    time: '时间',
    // Docker usage review (admin)
    dockerUsageReviewNav: 'Docker 复核',
    dockerUsageReviewTitle: 'Docker 使用复核',
    dockerUsageReviewDescription:
      '查看主机上未解析的 Docker/审计事件，并手动指定归属（覆盖自动归因，用于配额统计）。',
    dockerUsageReviewPickHost: '选择主机',
    dockerUsageReviewPickHostHint: '请选择主机以加载复核队列。',
    dockerUsageReviewEntityType: '实体类型',
    dockerUsageReviewContainers: '容器',
    dockerUsageReviewImages: '镜像',
    dockerUsageReviewVolumes: '数据卷',
    dockerUsageReviewQueueFailed: '加载复核队列失败',
    dockerUsageReviewEmpty: '当前筛选下没有未解析事件的实体。',
    dockerUsageReviewColId: 'ID',
    dockerUsageReviewColUnresolved: '未解析',
    dockerUsageReviewColAttribution: '归属',
    dockerUsageReviewDetails: '详情',
    dockerUsageReviewDrawerTitle: '实体详情',
    dockerUsageReviewCurrentAttribution: '当前归属',
    dockerUsageReviewIncludeUsed: '包含已用于自动归因的事件',
    dockerUsageReviewEventsFailed: '加载事件失败',
    dockerUsageReviewColSource: '来源',
    dockerUsageReviewColWhen: '时间',
    dockerUsageReviewColPayload: '载荷',
    dockerUsageReviewAssignee: '指派给已映射用户',
    dockerUsageReviewAssigneePlaceholder: '选择 OAuth → 主机用户映射',
    dockerUsageReviewNoMappings: '该主机暂无映射。请在「用户映射」中添加。',
    dockerUsageReviewCascade: '级联到相关镜像/层/卷（仅容器/镜像）',
    dockerUsageReviewAssign: '指派',
    dockerUsageReviewClear: '清除手动指派',
    dockerUsageReviewAssigned: '已保存手动归属',
    dockerUsageReviewAssignFailed: '指派失败',
    dockerUsageReviewCleared: '已清除手动归属',
    dockerUsageReviewClearFailed: '清除失败',
    dockerEventColReview: '复核',
    dockerEventColAuto: '自动归因',
    dockerEventReview_pending: '待处理',
    dockerEventReview_reviewed: '已复核',
    dockerEventAuto_unused: '未消费',
    dockerEventAuto_consumed: '已消费',
    dockerEventPayloadModalTitle: '事件载荷（JSON）',
    dockerEntityDetailTitle: 'Docker 实体详情',
    dockerEntityDetailTabInspect: 'Inspect',
    dockerEntityDetailTabAttribution: '归属',
    dockerEntityDetailTabEvents: '事件',
    dockerEntityDetailAutoAttribution: '自动归因（数据库）',
    dockerEntityDetailManualOverride: '手动覆盖',
    dockerEntityDetailNoAuto: '该实体没有自动归因记录。',
    dockerEntityDetailNoOverride: '无手动覆盖；有效配额仅使用自动归因。',
    dockerEntityDetailIncludeUsed: '包含已用于自动归因的事件',
    dockerEntityDetailIncludeResolved: '包含已从复核队列清除的事件',
    dockerEntityDetailInspectFailed: '加载 docker inspect 失败',
    dockerEntityDetailAttributionFailed: '加载归属详情失败',
    dockerEntityDetailEventsFailed: '加载使用事件失败',
    dockerEntityDetailDetails: '详情',
  },
}

type I18nContextValue = {
  locale: Locale
  setLocale: (locale: Locale) => void
  t: (key: string) => string
}

const I18nContext = createContext<I18nContextValue | null>(null)

export function I18nProvider({ children }: { children: ReactNode }) {
  const [locale, setLocaleState] = useState<Locale>(getDefaultLocale)

  const setLocale = useCallback((next: Locale) => {
    setLocaleState(next)
    localStorage.setItem(STORAGE_KEY, next)
  }, [])

  const t = useCallback(
    (key: string) => {
      const map = translations[locale]
      return map[key] ?? key
    },
    [locale]
  )

  const value = useMemo(
    () => ({ locale, setLocale, t }),
    [locale, setLocale, t]
  )

  return (
    <I18nContext.Provider value={value}>{children}</I18nContext.Provider>
  )
}

export function useI18n(): I18nContextValue {
  const ctx = useContext(I18nContext)
  if (!ctx) throw new Error('useI18n must be used within I18nProvider')
  return ctx
}
