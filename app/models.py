...
class AppConfig(BaseModel):
    ...
    DOCKER_QUOTA_AUTO_STOP_CONTAINERS: bool = True  # If false, do not stop containers when over Docker quota (monitoring-only)
    ...
    DOCKER_QUOTA_CACHE_TTL_SECONDS: int | None = None  # Container/image list Redis TTL (default 300s if unset; see cache.py)
    DOCKER_QUOTA_DF_CACHE_TTL_SECONDS: int | None = None  # get_system_df fresh TTL (default 300s if unset; see cache.py)
    DOCKER_QUOTA_DF_STALE_CACHE_TTL_SECONDS: int | None = None  # Max age to serve stale df snapshots on API paths (default 3600s)
